...
import { ContractsBlob, drawBeaconHandleDrawStartAndComplete, getContracts } from "./index";
...
export async function handler(event: RelayerParams): Promise<void> {
  ...
  try {
    const transactionPopulated: PopulatedTransaction | undefined = await drawBeaconHandleDrawStartAndComplete(
      contracts,
      {
        chainId,
        provider: signer,
    ...
    if (transactionPopulated) {
      ...
      throw new Error("DrawBeacon: transaction is not populated");
    }
  } catch (error) {
    console.log(error);
  }
