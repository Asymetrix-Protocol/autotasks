...
import { prizeFlush, getContracts, ContractsBlob } from "./index";
...
export async function handler(event: RelayerParams): Promise<void> {
  ...
  try {
    ...
    const transactionPopulated: PopulatedTransaction | undefined = await prizeFlush(contracts, {
      chainId,
      provider: signer,
    });
    ...
    if (transactionPopulated) {
      ...
      throw new Error("PrizeFlush: Transaction is not populated");
    }
  } catch (error) {
    console.log(error);
  }
